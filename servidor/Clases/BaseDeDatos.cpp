...
void BaseDeDatos::guardarCliente(SerializacionCliente cliente){
    std::lock_guard<std::mutex> lock (m);
    moverACliente(cliente.idCliente);
    //Omitir campos inmutables: Id, Raza y Clase.
	archivo.seekp(3*TAM_CADENAS, std::ios_base::cur);
	escribirCadena(cliente.idSala);
    archivo.write((char*)&cliente.infoPersonaje, sizeof(serializacionPersonaje));
}
